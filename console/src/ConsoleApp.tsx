...
import { AlertCircle, Box, CheckCircle2, ChevronDown, Copy, Download, FolderPlus, Github, Lock, LogOut, Menu, Plus, Settings2, Trash2, X } from "lucide-react";
...
function ProfileCard({ profile, onClick }: { profile: LauncherProfile; onClick: () => void }) {
  const count = profile.mods.length + profile.resourcePacks.length + profile.shaders.length;
  return <button type="button" className="profile-card-v2" style={{ "--accent": profile.accentColor } as React.CSSProperties} onClick={onClick}><span className="profile-dot" /><strong>{profile.name}</strong><p>{profile.customText}</p><span className="profile-meta"><b>MC {profile.minecraftVersion}</b><b>{profile.modLoader}</b><b>{count} files</b></span></button>;
}
...
function ProfileSettings({ profile, meta, errors, openPanels, setOpenPanels, onChange, onUpdateModpack }: { profile: LauncherProfile; meta: LauncherMeta | null; errors: string[]; openPanels: OpenPanels; setOpenPanels: (next: OpenPanels) => void; onChange: (profile: LauncherProfile) => void; onUpdateModpack: () => void }) {
...
export function ConsoleApp() {
  ...
  async function reload() { setStatusText("불러오는 중..."); const result = await loadProfiles(); setProfiles(result.profiles); setSelectedId(result.profiles[0]?.id ?? ""); setDirty(false); setView("home"); setStatusText(result.source === "local" ? "서버 로컬 저장소에서 불러옴" : "아직 저장된 프로필 없음"); }
