...
import {
  ...
  getLatestModrinthAsset,
  getModrinthAsset,
  getModrinthVersionOptions,
...
type OpenPanels = { profile: boolean; permissions: boolean; validation: boolean };

const PAGE_SIZE = 12;
const contentSections: Record<AssetKind, { title: string; projectKind: ProjectKind; accept: string }> = {
  mods: { title: "모드", projectKind: "mod", accept: ".jar" },
  resourcePacks: { title: "리소스팩", projectKind: "resourcepack", accept: ".zip" },
  shaders: { title: "쉐이더", projectKind: "shader", accept: ".zip" },
};
...
function safeId(value: string) { return value.toLowerCase().trim().replace(/[^a-z0-9_-]+/g, "-").replace(/^-+|-+$/g, "") || `profile-${Date.now()}`; }
...
function serverInput(profile: LauncherProfile) { return profile.defaultServer.port === 25565 ? profile.defaultServer.address : `${profile.defaultServer.address}:${profile.defaultServer.port}`; }
function newestLoader(meta: LauncherMeta | null, loader: LauncherProfile["modLoader"]) { return loader === "vanilla" ? "" : meta?.loaders[loader]?.[0] ?? ""; }

function Field({ label, children }: { label: string; children: React.ReactNode }) { return <label className="ui-field"><span>{label}</span>{children}</label>; }
...
function LoginScreen({ onDone }: { onDone: () => void }) {
  ...
  const submit = async (event: React.FormEvent) => { event.preventDefault(); setBusy(true); setError(""); try { await login(username, secret); onDone(); } catch (reason) { setError(reason instanceof Error ? reason.message : "로그인 실패"); } finally { setBusy(false); } };
...
function ProfileCard({ profile, active, onClick }: { profile: LauncherProfile; active?: boolean; onClick: () => void }) {
  ...
  return <button type="button" className={`profile-card-v2${active ? " active" : ""}`} style={{ "--accent": profile.accentColor } as React.CSSProperties} onClick={onClick}><span className="profile-dot" /><strong>{profile.name}</strong><p>{profile.customText}</p><span className="profile-meta"><b>MC {profile.minecraftVersion}</b><b>{profile.modLoader}</b><b>{count} files</b></span></button>;
}
function HomeView({ profiles, onOpen, onNew }: { profiles: ProfilesManifest; onOpen: (id: string) => void; onNew: () => void }) {
  return <section className="home-only-panel"><div className="home-title"><p className="eyebrow">Profiles</p><h2>프로필</h2><span>{profiles.length}개</span></div><div className="home-profile-grid">{profiles.map((profile) => <ProfileCard key={profile.id} profile={profile} onClick={() => onOpen(profile.id)} />)}<button className="add-profile-card home-add-card" onClick={onNew}><Plus /><span>새 프로필</span></button></div></section>;
}
function NewProfileChoice({ onCustom, onModpack, onBack }: { onCustom: () => void; onModpack: () => void; onBack: () => void }) {
  return <section className="new-choice-panel"><div className="home-title"><p className="eyebrow">Create</p><h2>새 프로필</h2><span>시작 방식 선택</span></div><div className="new-choice-grid"><button onClick={onCustom}><strong>커스텀</strong><p>최신 마크/로더를 기본값으로 두고 직접 구성합니다.</p></button><button onClick={onModpack}><strong>모드팩</strong><p>Modrinth 모드팩을 가져와 내부 파일을 분해해서 넣습니다.</p></button></div><button className="ghost-button" onClick={onBack}>홈으로</button></section>;
}

function ProfileSettings({ profile, meta, validationErrors, openPanels, setOpenPanels, onChange, onUpdateModpack }: { profile: LauncherProfile; meta: LauncherMeta | null; validationErrors: string[]; openPanels: OpenPanels; setOpenPanels: (next: OpenPanels) => void; onChange: (profile: LauncherProfile) => void; onUpdateModpack: () => void }) {
  ...
  const setLoader = (modLoader: LauncherProfile["modLoader"]) => update({ modLoader, modLoaderVersion: newestLoader(meta, modLoader) });
  ...
  useEffect(() => {
    const latest = newestLoader(meta, profile.modLoader);
    if (latest && profile.modLoaderVersion !== latest) update({ modLoaderVersion: latest });
  }, [meta?.loaders.fabric?.[0], meta?.loaders.forge?.[0], meta?.loaders.quilt?.[0], profile.modLoader]);

  return <section className="settings-column" id="profile-settings-section">
    <Collapsible title={<><Settings2 size={18} />프로필 설정</>} subtitle="실행 기준" open={openPanels.profile} onToggle={() => setOpenPanels({ ...openPanels, profile: !openPanels.profile })}>
      <div className="form-stack">
        ...
        <div className="split-grid"><Field label="마크 버전"><select value={profile.minecraftVersion} onChange={(event) => setMcVersion(event.target.value)}>{(meta?.minecraft.releases ?? [profile.minecraftVersion]).map((version) => <option key={version}>{version}</option>)}</select></Field><Field label="Java"><input value={`Java ${profile.javaVersion}`} readOnly /></Field></div>
        <div className="split-grid"><Field label="로더"><select value={profile.modLoader} onChange={(event) => setLoader(event.target.value as LauncherProfile["modLoader"])}>{MOD_LOADERS.map((loader) => <option key={loader}>{loader}</option>)}</select></Field><Field label="로더 버전"><select value={profile.modLoaderVersion} onChange={(event) => update({ modLoaderVersion: event.target.value })}>{profile.modLoader === "vanilla" ? <option value="">Vanilla</option> : (meta?.loaders[profile.modLoader] ?? [profile.modLoaderVersion]).map((version) => <option key={version}>{version}</option>)}</select></Field></div>
        <Field label="서버 주소"><input value={serverInput(profile)} placeholder="mc.zzapcho.kr 또는 mc.zzapcho.kr:25565" onChange={(event) => setServer(event.target.value)} /></Field>
        <Field label={`RAM ${ram}GB`}><div className="ram-control"><input type="range" min={1} max={16} step={0.5} value={ram} onChange={(event) => setMemoryGb(Number(event.target.value))} /><input type="number" min={1} max={64} step={0.5} value={ram} onChange={(event) => setMemoryGb(Number(event.target.value))} /></div></Field>
        ...
        {profile.modpack && <button className="secondary-button modpack-update-button" type="button" onClick={onUpdateModpack}><Download size={16} />모드팩 업데이트 하기</button>}
      </div>
    </Collapsible>
    <Collapsible title={<><Lock size={18} />런처 수정 허용</>} subtitle="true = 유저 수정 가능" open={openPanels.permissions} onToggle={() => setOpenPanels({ ...openPanels, permissions: !openPanels.permissions })}>
    ...
    <Collapsible title={<>{validationErrors.length ? <AlertCircle size={18} /> : <CheckCircle2 size={18} />}검증</>} subtitle={validationErrors.length ? `${validationErrors.length}개 문제` : "저장 가능"} open={openPanels.validation} onToggle={() => setOpenPanels({ ...openPanels, validation: !openPanels.validation })}>
      {validationErrors.length ? <ul className="errors-v2 detailed-errors">{validationErrors.map((error) => <li key={error}>{error}</li>)}</ul> : <p className="validation-ok-text">문제 없음. 저장 가능한 상태입니다.</p>}
    </Collapsible>
  </section>;
}
...
function ContentLibraryPanel({ profile, kind, onChange }: { profile: LauncherProfile; kind: AssetKind; onChange: (profile: LauncherProfile) => void }) {
  ...
  const loadProjects = async (term = query, reset = true) => {
    ...
    try {
      const next = source === "modrinth" ? await searchModrinthProjects(section.projectKind, term, offset, PAGE_SIZE) : await searchCurseForgeProjects(section.projectKind, term || section.title);
    ...
    } catch (error) { if (reset) setResults([]); setHasMore(false); setMessage(error instanceof Error ? error.message : "검색 실패"); } finally { setBusy(false); }
  };
...
function ModpackPicker({ onBack, onCreate }: { onBack: () => void; onCreate: (profile: LauncherProfile) => void }) {
  ...
  const [hasMore, setHasMore] = useState(true);
  const load = async (term = query, reset = true) => { if (busy) return; setBusy(true); const offset = reset ? 0 : results.length; setMessage(term.trim() ? "검색 중..." : "인기 모드팩"); try { const next = await searchModrinthProjects("modpack", term, offset, PAGE_SIZE); setResults((current) => reset ? next : [...current, ...next.filter((item) => !current.some((old) => old.projectId === item.projectId))]); setHasMore(next.length >= PAGE_SIZE); } catch (error) { if (reset) setResults([]); setHasMore(false); setMessage(error instanceof Error ? error.message : "검색 실패"); } finally { setBusy(false); } };
  ...
  const select = async (project: ExternalProject) => { setBusy(true); setMessage(".mrpack 분석 중..."); try { onCreate(await createProfileFromModrinthModpack(project)); } catch (error) { setMessage(error instanceof Error ? error.message : "모드팩 분석 실패"); } finally { setBusy(false); } };
...
export function ConsoleApp() {
  ...
  const [openPanels, setOpenPanels] = useState<OpenPanels>({ profile: true, permissions: true, validation: true });
  ...
  const refreshMetaForVersion = async (version: string) => { try { setMeta(await loadLauncherMeta(version)); } catch { /* ignore */ } };
  const reload = async () => { setStatus("불러오는 중..."); const result = await loadProfiles(); setProfiles(result.profiles); setSelectedId(result.profiles[0]?.id ?? ""); setDirty(false); setView("home"); setStatus(result.source === "local" ? "서버 로컬 저장소에서 불러옴" : "아직 저장된 프로필 없음"); };
  useEffect(() => { if (sessionReady) reload().catch((error) => setStatus(error instanceof Error ? error.message : "불러오기 실패")); }, [sessionReady]);
  useEffect(() => { if (selected?.minecraftVersion) void refreshMetaForVersion(selected.minecraftVersion); }, [selected?.minecraftVersion]);
  ...
  const createCustomProfile = () => { const profile = createEmptyProfile(); if (meta?.minecraft.latestRelease) { profile.minecraftVersion = meta.minecraft.latestRelease; profile.javaVersion = guessJavaVersion(profile.minecraftVersion); profile.modLoaderVersion = newestLoader(meta, profile.modLoader); } setProfiles((items) => [...items, profile]); setSelectedId(profile.id); setView("editor"); setDirty(true); };
  const createModpackProfile = (profile: LauncherProfile) => { setProfiles((items) => [...items, profile]); setSelectedId(profile.id); setView("editor"); setDirty(true); };
  const duplicate = () => { if (!selected) return; const profile = clone(selected); profile.id = `${selected.id}-copy-${Date.now().toString().slice(-4)}`; profile.name = `${selected.name} Copy`; setProfiles((items) => [...items, profile]); setSelectedId(profile.id); setView("editor"); setDirty(true); };
  const confirmRemove = () => { if (!deleteTarget) return; setProfiles((items) => items.filter((item) => item.id !== deleteTarget.id)); setSelectedId(""); setDeleteTarget(null); setView("home"); setDirty(true); setStatus("프로필 삭제됨. 저장하면 서버 업로드 파일도 정리됩니다."); };
  const exportJson = () => { void navigator.clipboard.writeText(JSON.stringify(profiles, null, 2)); setStatus("JSON 클립보드 복사 완료"); };
  const updateModpack = async () => { if (!selected?.modpack) return; setStatus("모드팩 업데이트 확인 중..."); const project: ExternalProject = { source: selected.modpack.source, projectId: selected.modpack.projectId, slug: selected.modpack.slug, title: selected.modpack.title, description: selected.description, projectType: "modpack" }; try { const updated = await createProfileFromModrinthModpack(project); setProfile({ ...updated, id: selected.id, defaultServer: selected.defaultServer, launchOptions: selected.launchOptions, editableFields: selected.editableFields }); setStatus("모드팩 업데이트 적용됨"); } catch (error) { setStatus(error instanceof Error ? error.message : "모드팩 업데이트 실패"); } };
  const save = async () => { if (!validation.ok) { setSaveStatus({ type: "error", message: "검증 오류" }); return; } setIsSaving(true); setStatus("서버에 저장 중..."); try { const result = await saveProfiles(profiles); setDirty(false); const deletedText = result.deletedProfileUploads ? ` · 삭제 프로필 파일 ${result.deletedProfileUploads}개 정리` : ""; setStatus(`서버 저장 완료${deletedText}`); setSaveStatus({ type: "success", message: "저장됨", sha: result.sha }); } catch (error) { const message = error instanceof Error ? error.message : "저장 실패"; setStatus(message); setSaveStatus({ type: "error", message }); } finally { setIsSaving(false); } };

  if (!sessionReady) return <LoginScreen onDone={() => setSessionReady(true)} />;
  return <main className="app-v2 notion-bg"><TopBar view={view} dirty={dirty} saveStatus={saveStatus} isSaving={isSaving} canSave={validation.ok} onHome={() => setView("home")} onSave={save} onMenu={() => setMenuOpen(true)} />{menuOpen && <div className="mobile-sheet-backdrop" onClick={() => setMenuOpen(false)}><div className="mobile-sheet" onClick={(event) => event.stopPropagation()}><button className="sheet-close" onClick={() => setMenuOpen(false)}><X /></button><button onClick={exportJson}><Copy size={16} />JSON 복사</button><button onClick={() => { clearSession(); setSessionReady(false); }}><LogOut size={16} />나가기</button></div></div>}{deleteTarget && <ConfirmDeleteModal profile={deleteTarget} onCancel={() => setDeleteTarget(null)} onConfirm={confirmRemove} />}{view === "home" && <HomeView profiles={profiles} onNew={() => setView("new")} onOpen={(id) => { setSelectedId(id); setView("editor"); }} />}{view === "new" && <NewProfileChoice onCustom={createCustomProfile} onModpack={() => setView("modpack")} onBack={() => setView("home")} />}{view === "modpack" && <ModpackPicker onBack={() => setView("new")} onCreate={createModpackProfile} />}{view === "editor" && selected && <><section className="profile-section-v2 compact-profile-header"><div className="section-title-row"><h2><Box size={18} />{selected.name}</h2><span>{dirty ? "수정됨" : "동기화됨"}</span></div><div className="quick-actions-v2"><button onClick={duplicate}>복제</button><button className="danger-text" onClick={() => setDeleteTarget(selected)}>삭제</button></div></section><div className="workspace-v2 launcher-style-workspace editor-no-page-scroll"><ProfileSettings profile={selected} meta={meta} validationErrors={validation.errors} openPanels={openPanels} setOpenPanels={setOpenPanels} onChange={setProfile} onUpdateModpack={() => void updateModpack()} /><section className="content-column content-column-fixed"><div className="notion-card tab-shell-shot">{tabs.map((tab) => <button key={tab.id} className={activeTab === tab.id ? "active" : ""} onClick={() => setActiveTab(tab.id)}>{tab.label}<b>{selected[tab.id].length}</b></button>)}</div><ContentLibraryPanel profile={selected} kind={activeTab} onChange={setProfile} /></section></div></>}</main>;
}
