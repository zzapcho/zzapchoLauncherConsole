...
function ProfileCard({ profile, index, total, reorderMode, onClick, onMove }: { profile: LauncherProfile; index: number; total: number; reorderMode: boolean; onClick: () => void; onMove: (index: number, offset: -1 | 1) => void }) {
  ...
  return <article className={`profile-card-frame ${reorderMode ? 'is-reordering' : ''}`} style={{ '--accent': profile.accentColor } as React.CSSProperties}><button type='button' className='profile-card-v2 profile-open-button' onClick={onClick} disabled={reorderMode}><span className='profile-dot' /><strong>{profile.name}</strong><p>{profile.customText}</p><span className='profile-meta'><b>MC {profile.minecraftVersion}</b><b>{profile.modLoader}</b><b>{count} files</b></span></button>{reorderMode && <div className='profile-order-actions' aria-label='프로필 순서 변경'><button type='button' disabled={index === 0} onClick={() => onMove(index, -1)} title='위로'><ArrowUp size={14} /><span>위</span></button><button type='button' disabled={index === total - 1} onClick={() => onMove(index, 1)} title='아래로'><ArrowDown size={14} /><span>아래</span></button></div>}</article>;
}
function HomeView({ profiles, reorderMode, onOpen, onNew, onMove, onToggleReorder }: { profiles: ProfilesManifest; reorderMode: boolean; onOpen: (id: string) => void; onNew: () => void; onMove: (index: number, offset: -1 | 1) => void; onToggleReorder: () => void }) {
  return <section className={`home-only-panel ${reorderMode ? 'home-reorder-mode' : ''}`}><div className='home-title'><p className='eyebrow'>Profiles</p><h2>프로필</h2><div className='home-title-actions'><span>{profiles.length}개</span><button type='button' className={`reorder-mode-button ${reorderMode ? 'active' : ''}`} onClick={onToggleReorder}>{reorderMode ? '완료' : '위치 변경'}</button></div></div><div className='home-profile-grid'>{profiles.map((profile, index) => <ProfileCard key={profile.id} profile={profile} index={index} total={profiles.length} reorderMode={reorderMode} onClick={() => onOpen(profile.id)} onMove={onMove} />)}{!reorderMode && <button className='add-profile-card home-add-card' onClick={onNew}><Plus /><span>새 프로필</span></button>}</div></section>;
}
